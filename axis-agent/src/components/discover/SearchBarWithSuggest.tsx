...
import { PredictionGroup } from '../create/manual/PredictionEventCard';
...
export const SearchBarWithSuggest: React.FC<SearchBarWithSuggestProps> = ({
  ...
  const searchResults = value.trim()
    ? allMarkets.filter(m =>
        m.question.toLowerCase().includes(value.toLowerCase())
      ).slice(0, 5)
    : [];
                ...
                <>
                  ...
                  {searchResults.map((group, idx) => (
                    ...
                    >
                      ...
                      <div className={styles.suggestionText}>{group.question}</div>
                    </button>
                  ))}
                </>
              ) : (
